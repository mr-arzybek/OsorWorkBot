...
from db.sql_commands.utils import update_product_coming_quantity
...
async def load_submit(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        if message.text.lower() == 'да':
            ...
            await state.finish()
            await update_product_coming_quantity(cursor=connection, quantity=data['quantity'], articul=data['articul'])
