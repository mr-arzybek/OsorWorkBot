...
async def load_articul(message: types.Message, state: FSMContext):
    if message.text.isdigit():
        ...
        product_coming_data = get_product_from_articul(cursor=connection, articul=data['articul'], city=data['city'])
...
async def load_submit(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        if message.text.lower() == 'да':
            update_product_coming_quantity(connection, data['quantity'], data['articul'])
